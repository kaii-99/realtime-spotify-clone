...
df = df[["song_id", "genre", "language", "type"]]
...
onehot_df["song_id"] = df["song_id"].values

# Aggregate by song_id (if a song appears multiple times)
song_features = onehot_df.groupby("song_id").mean().reset_index()

# Keep song_id separately
song_ids = song_features["song_id"].values

# Feature matrix (only numeric features)
song_features_matrix = song_features.drop(columns=["song_id"]).values
...
sns.heatmap(
    similarity_matrix,
    xticklabels=song_features["song_id"],
    yticklabels=song_features["song_id"],
    cmap="viridis"
)
