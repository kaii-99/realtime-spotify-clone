...
def get_user_preferences(user_id_enc):
    # Use listening history
    user_history = processed_data[processed_data["user_id_encoded"] == user_id_enc]

    # Preferred languages
    preferred_langs = user_history["language_encoded"].unique()

    # Top 3 genres
    top_genres = user_history["genre_encoded"].value_counts().head(3).index.tolist()
...
def filter_songs_for_user(user_id, weather, timeOfDay):
    ...
    preferred_langs, top_genres = get_user_preferences(user_id_enc)
...
def decode_features(row):
    return {
        "User ID": user_enc.inverse_transform([int(row[0])])[0],
        "Genre": genre_enc.inverse_transform([int(row[1])])[0],
        "Language": language_enc.inverse_transform([int(row[2])])[0],
        "Type": type_enc.inverse_transform([int(row[3])])[0],
        "Weather": weather_enc.inverse_transform([int(row[4])])[0],
        "Time of Day": time_enc.inverse_transform([int(row[5])])[0],
    }
...
def recommend_and_explain(user_id, weather, timeOfDay, top_n=5):
    candidate_songs, inputs_df = filter_songs_for_user(user_id, weather, timeOfDay)
