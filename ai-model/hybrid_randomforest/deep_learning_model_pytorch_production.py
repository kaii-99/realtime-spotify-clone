...
def load_user_model(user_id):
    model_path = f"hybrid_randomforest/model/{user_id}/{user_id}_personalized_model_pytorch.pth"
    song_map_path = f"hybrid_randomforest/model/{user_id}/{user_id}_song_mapping.json"
...
def encode_context_for_user(user_id, weather, timeOfDay):
    base = f"hybrid_randomforest/processed_data/{user_id}"
    weather_enc = joblib.load(f"{base}/weather_encoder.pkl")
    time_enc = joblib.load(f"{base}/timeofday_encoder.pkl")

    w = weather_enc.transform([weather])[0] if weather in weather_enc.classes_ else 0
    t = time_enc.transform([timeOfDay])[0] if timeOfDay in time_enc.classes_ else 0
...
def group_recommendation(user_ids, all_songs_df, weather, timeOfDay, alpha=0.7, top_n=10):
    score_list = []

    for uid in user_ids:
        model, song_map = load_user_model(uid)

        weather_val, time_val = encode_context_for_user(
            uid, weather, timeOfDay
        )

        songs = all_songs_df.copy()
        songs["weather_encoded"] = weather_val
        songs["timeOfDay_encoded"] = time_val

        X = torch.tensor(songs[["genre_encoded", "language_encoded", "type_encoded","weather_encoded","timeOfDay_encoded"]].values, dtype=torch.float32)
        with torch.no_grad():
...
def hybrid_recommendation_DL(group_id, weather, timeOfDay):
