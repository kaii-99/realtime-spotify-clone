...
def explain_song_for_user(user_id, song_row, weather, timeOfDay):
    set_files(user_id)

    weather_enc_val = safe_transform(weather_enc, weather)
    time_enc_val = safe_transform(time_enc, timeOfDay)
...
def recommend_group_playlist(group_id, weather, timeOfDay, top_n=10, alpha=0.7):
...
def explain_group_song(group_id, song_id, weather, timeOfDay, alpha=0.7):
    members = fetch_group_members(group_id)
    ...
    for uid in members:
        try:
            exp = explain_song_for_user(uid, song_row, weather, timeOfDay)
