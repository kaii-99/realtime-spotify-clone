...
def recommend_songs(model, all_songs_df, weather_enc_val, time_enc_val, top_n=5, explain=False):
    """
    ...
    if explain:
        ...
        shap_values = explainer.shap_values(X[:top_n])
        
        # Add SHAP values to the DataFrame for easy access
        top_songs["shap_values"] = [sv.tolist() for sv in shap_values[0]]  # using first output class
...
for _, row in unique_conditions.iterrows():
    ...
    top_songs = recommend_songs(model, all_songs, weather_val, time_val, top_n=5, explain=True)
    ...
    for i, song_row in top_songs.iterrows():
