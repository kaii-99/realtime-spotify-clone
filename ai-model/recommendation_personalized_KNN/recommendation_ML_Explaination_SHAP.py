...
def set_files(user_id):
    ...
    background = processed_data[[
        "genre_encoded",
        "language_encoded",
        "type_encoded",
        "weather_encoded",
        "timeOfDay_encoded"
    ]].sample(min(50, len(processed_data)))
...
def score_songs(candidate_songs, inputs_df):
    # probability of like
    probs_all = model.predict_proba(inputs_df)  # shape: (num_samples, num_classes)
    pred_classes = np.argmax(probs_all, axis=1)
    probs = probs_all[np.arange(len(inputs_df)), pred_classes]  # probability of the predicted class
...
def explain_top_songs(candidate_songs, inputs_df, top_n=5):
    ...
    for pos, song in top_songs.iterrows():
        shap_values = explainer.shap_values(inputs_df.iloc[pos:pos+1])
        predicted_class = np.argmax(model.predict_proba(inputs_df.iloc[pos:pos+1]))
